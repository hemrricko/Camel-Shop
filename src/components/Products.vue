<template>
  <h1>Confectionery</h1>
  <p>{{total}}</p>
      <div>
      <v-row>
          <v-col cols="12" md="3"  v-for="product in stockDetails" :key=product>
      <v-card
      class="mx-auto my-12"
      max-width="374"
  >
      <v-img
      height="250"
      :src="product.itemImage"
      cover
      ></v-img>

      <v-card-item>
      <v-card-title>{{ product.itemName }}</v-card-title>
      </v-card-item>

      <v-card-text>
      <div class="my-4 text-subtitle-1">
          {{ product.itemPrice }}
      </div>

      <div>{{ product.description }}</div>
      </v-card-text>

      <v-divider class="mx-4 mb-1"></v-divider>

      <v-card-actions>
      <v-btn
          color="#F0F"
          text="Add to Cart"
          block
          border
          @click="addToCart(product)"
      ></v-btn>
      </v-card-actions>
  </v-card>
  </v-col>
  </v-row>
</div>
</template>
<script>
import { mapActions, mapGetters } from 'vuex'

export default{
  data(){
      return{
              stockDetails : [
          {
                itemNumber: 1,
                itemName : "Sweets",
                itemPrice : 5,
                itemQuantity : 30,
                itemImage: '../images/customerbox-aXq1oCCjlVM-unsplash.jpg',
                description: "Our sweets are wonderfully crafted by the best ingrdients to bring deliciousness to your parties"
            },
            {
                itemNumber: 98,
                itemName : "Cup Cakes",
                itemPrice : 50,
                itemQuantity : 30,
                itemImage: 'public/images/leo-roza-VkMrtmuy9mY-unsplash.jpg',
                description: "Our sweets are wonderfully crafted by the best ingrdients to bring deliciousness to your parties"
            },
            {
                itemNumber: 32,
                itemName : "Sweet Cake",
                itemPrice : 5,
                itemQuantity : 30,
                itemImage: 'public/images/bao-menglong-j9oQ7nbKYNI-unsplash.jpg',
                description: "Our sweet Cakes are wonderfully crafted by the best ingrdients to bring deliciousness to your parties"
            },
            {
                itemNumber: 27466,
                itemName : "Lollipop",
                itemPrice : 10,
                itemQuantity : 4,
                itemImage: '..//images/arturo-esparza-Ky4q3IbRCLk-unsplash.jpg',
                description: "Our Lollipops are wonderfully crafted by the best ingrdients to bring deliciousness to your parties"
            },
     {
                itemNumber: 273,
                itemName : "Chocolate",
                itemPrice : 250,
                itemQuantity : 70,
                itemImage: 'public/images/tetiana-bykovets-TzN2odwnesg-unsplash.jpg',
                description: "Our Chocolates are wonderfully crafted by the best ingrdients to bring deliciousness to your parties"
            },
     {
                itemNumber: 1,
                itemName : "Diz Nuts",
                itemPrice : 5,
                itemQuantity : 30,
                itemImage: 'public/images/merve-aydin-jD2dIC2IZME-unsplash.jpg',
                description: "Our Coated Nutsare wonderfully crafted by the best ingrdients to bring deliciousness to your parties"
            },
     {
                itemNumber: 27,
                itemName : "Smocha",
                itemPrice : 70,
                itemQuantity : 100,
                itemImage: 'public/images/Kenyan wrap_ Smocha _ chapo smokie.jpeg',
                description: "Our Smocha's are wonderfully crafted by the best ingrdients to bring deliciousness to your parties"
            }
      ],
      total:0,

      cartItems:[],
      }
  },

  methods:{
      calculateTotal(itemPrice){
          this.total+= itemPrice

      },
      addToCart(product){
          this.cartItems.push(product)
          this.calculateTotal(product.itemPrice)

      },
      removeFromCart(product){
          this.cartItems.pop(product)
          this.total -= product.itemPrice
      }

   },
computed: {
    ...mapGetters({
        stock: 'stock'
        
    })
}
}
</script>
